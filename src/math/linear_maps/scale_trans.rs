...
use super::{
  translate::TranslateTransform, uniform_scale::UniformScaleTransform, MatrixTransform, Transform
};
use crate::{
  math::*,
...
pub struct ScaleTranslate<In: Space<3>, Middle: Space<3>, Out: Space<3>> {
  scale: UniformScaleTransform<In, Middle>,
  translate: TranslateTransform<Middle, Out>
}

impl<In: Space<3>, Middle: Space<3>, Out: Space<3>> ScaleTranslate<In, Middle, Out> {
  pub fn new(
    scale: UniformScaleTransform<In, Middle>,
    translate: TranslateTransform<Middle, Out>
  ) -> Self {
...
impl<In: Space<3>, Middle: Space<3>, Out: Space<3>> Transform<In, Out>
  for ScaleTranslate<In, Middle, Out>
{
  fn identity() -> Self
  where Self: Sized {
    Self::new(UniformScaleTransform::identity(), TranslateTransform::identity())
  }

  fn matrix(&self) -> MatrixTransform<In, Out> { self.translate.matrix() * self.scale.matrix() }

  fn vector(&self, vector: &Vector3<In>) -> Vector3<Out> {
    self.translate.vector(&self.scale.vector(vector))
  }

  fn point(&self, point: &Point3<In>) -> Point3<Out> {
    self.translate.point(&self.scale.point(point))
  }

  fn direction(&self, dir: &UnitVector3<In>) -> UnitVector3<Out> {
    self.translate.direction(&self.scale.direction(dir))
  }

  fn normal(&self, sn: &UnitVector3<In>) -> UnitVector3<Out> {
    self.translate.normal(&self.scale.normal(sn))
  }
  ...
  fn inverse_vector(&self, vector: &Vector3<Out>) -> Vector3<In> {
    self.scale.inverse_vector(&self.translate.inverse_vector(vector))
  }

  fn inverse_point(&self, point: &Point3<Out>) -> Point3<In> {
    self.scale.inverse_point(&self.translate.inverse_point(point))
  }

  fn inverse_direction(&self, dir: &UnitVector3<Out>) -> UnitVector3<In> {
    self.scale.inverse_direction(&self.translate.inverse_direction(dir))
  }

  fn inverse_normal(&self, sn: &UnitVector3<Out>) -> UnitVector3<In> {
    self.scale.inverse_normal(&self.translate.inverse_normal(sn))
  }

  fn inverse_ray(&self, ray: &Ray3<Out>) -> Ray3<In> {
    self.scale.inverse_ray(&self.translate.inverse_ray(ray))
  }
